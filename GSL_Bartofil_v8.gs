...
function corrigirFuso() {
  ...
  ui.alert('FUSO E FORMATO DE DATA\n\n' +
    'Fuso antes: ' + (atual || '(vazio — era esse o problema)') + '\n' +
    'Fuso agora: ' + FUSO_PADRAO + '\n\n' +
    'Datas em dd/mm/aaaa: ' + tratadas + ' aba(s) mensal(is)' +
    (arrumados ? '\nPrazos com hora corrigidos (voltam ao calendário): ' + arrumados : '') +
    (modelo ? ' + MODELO_MES' : ' (MODELO_MES não encontrada!)') + '\n\n' +
    'Confira também em Arquivo > Configurações da planilha > Fuso horário, ' +
    'e no editor do Apps Script em Configurações do projeto > Fuso horário.');
}
