...
function acaoFinalizarEntrega(usuario, params) {
  ...
  try {
    ...
    if (pdfs.length && (docs.length || imgs.length || pdfs.length > 1)) {
      throw new Error('Um PDF ja pronto nao pode ser juntado com outros arquivos. ' +
    ...
    atualizar('ATIVIDADES', params.id, {
      ANEXOS: url, ENTREGUE_EM: agoraTexto(), STATUS: STATUS.AGUARDANDO
    }, usuario.email);

    var atualizada = hidratar(obter('ATIVIDADES', params.id));
    try {
...
function _viaMiniaturaDrive(file, largura) {
  ...
  if (!link) {
    throw new Error('O Google ainda nao gerou a pre-visualizacao de "' + file.getName() +
  ...
  if (img.getResponseCode() !== 200) {
    throw new Error('Nao consegui converter "' + file.getName() + '" (HTTP ' + img.getResponseCode() + ').');
  }
...
function _paraGoogleDocEntrega(blob, nome) {
  ...
  if (r.getResponseCode() !== 200) {
    throw new Error('Nao consegui converter "' + blob.getName() + '" (HTTP ' + r.getResponseCode() +
