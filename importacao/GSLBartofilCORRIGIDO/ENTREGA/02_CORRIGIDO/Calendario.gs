...
function acaoValidar(usuario, params) {
  ...
  try { avisarValidacao(hidratar(comCampos_(antes, campos)), validacao, usuario); } catch (e) {}
  return { ok: true };
}
...
function acaoDefinirSetor(usuario, params) {
  ...
  atualizar('ATIVIDADES', params.id, campos, usuario.email);
  try { avisarSetorDefinido(hidratar(comCampos_(antes, campos))); } catch (e) {}
  return { ok: true };
}
...
function acaoRemarcar(usuario, params) {
  ...
  if (params.avisar !== false) {
    try { avisarRemarcacao(hidratar(comCampos_(antes, campos)), prazoAntigo, motivo); } catch (e) {}
  }
  return { ok: true };
}
...
function acaoCancelarAtividade(usuario, params) {
  ...
  try { avisarValidacao(hidratar(comCampos_(a, campos)), 'Cancelada', usuario); } catch (e) {}
  return { ok: true };
}
...
function acaoCriarAtividade(usuario, params) {
  ...
  try { avisarNovaAtividade(hidratar(nova)); } catch (e) {}
