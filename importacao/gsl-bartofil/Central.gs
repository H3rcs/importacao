...
function gerarInsights(todas) {
  ...
  const jan56 = linhas.filter(function (a) { return a.prazoNum > hojeN - 56; });
  const jan84 = linhas.filter(function (a) { return a.prazoNum > hojeN - 84; });
  ...
  const parados = linhas.filter(function (a) {
    return a.status === STATUS.AGUARDANDO && a.prazoNum < hojeN - 5;
  });
  ...
  TURNOS.forEach(function (t) {
    const base = jan84.filter(function (a) {
      return doTurno(a, t) && a.prazoNum <= hojeN &&
